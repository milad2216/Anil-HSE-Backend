...
using Anil.Data.DataProviders.LinqToDB;
using Anil.Data.Migrations;
using Npgsql;

namespace Anil.Data.DataProviders
{
    public partial class PostgreSqlDataProvider : BaseDataProvider, IAnilDataProvider
    {
        #region Fields
        ...
        private string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : BaseEntity
        {
            ...
            if (descriptor is null)
                throw new AnilException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
            ...
            if (string.IsNullOrEmpty(columnName))
                throw new AnilException("A table's primary key does not have an identity constraint");
        ...
        public virtual string BuildConnectionString(IAnilConnectionStringInfo nopConnectionString)
        {
            if (nopConnectionString is null)
                throw new ArgumentNullException(nameof(nopConnectionString));

            if (nopConnectionString.IntegratedSecurity)
                throw new AnilException("Data provider supports connection only with login and password");
